...
public class PaginationTaglib extends SimpleTagSupport {
	...
	public void doTag() throws JspException {
		...
		try {
			...
			for(int itr=0;itr<count;itr+=steps) {
				if(offset==itr)
					out.write(constructLink((itr/100+1)-1 *steps, String.valueOf(itr/100+1), "activeOne", true));
				else
					out.write(constructLink(itr/100*steps, String.valueOf(itr/100+1), null , false));
			}
	...
	private String constructLink(int page, String text, String className, boolean disabled) {
		...
		if (className != null) {
			link.append(" class=\"");
			link.append(className);//+" "+"style="+"background-color:#ffcc00;");
		//	background: #ff9999;
			link.append("\"");
		}
		if(disabled)
